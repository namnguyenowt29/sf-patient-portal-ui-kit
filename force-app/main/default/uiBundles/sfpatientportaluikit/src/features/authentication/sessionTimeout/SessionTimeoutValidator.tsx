 ...
 * @internal
 */
function SessionExpiredAlert({ show, onDismiss, message = LABELS.invalidSessionMessage }: SessionExpiredAlertProps) {
  if (!show) {
    return null;
  }
      ...
      <Alert variant="destructive" role="alert">
        <AlertTitle className="flex items-center justify-between">
          ...
          <Button
            ...
            className="hover:bg-destructive/10 h-auto p-1"
          >
            <X className="h-4 w-4" />
          </Button>
        </AlertTitle>
    ...
    <Dialog open={isOpen} onOpenChange={() => {}}>
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription id="session-warning-description">{LABELS.sessionWarningMessage}</DialogDescription>
        </DialogHeader>
        ...
        <div className="flex flex-col items-center gap-4 py-6">
          ...
          <time
            ...
            className="text-destructive text-center text-5xl font-bold tabular-nums"
          >
            {formattedTime}
          </time>
...
export default function SessionTimeoutValidator({
  ...
  const [showExpiredAlert, setShowExpiredAlert] = useState(() => {
    ...
    const shouldShow = isLoginPage && sessionStorage.getItem(STORAGE_KEYS.SHOW_SESSION_MESSAGE) === "true";
    if (shouldShow) {
   ...
   */
  useEffect(() => {
    if (sessionTimeout.timeLeftInSession !== null && sessionTimeout.timeLeftInSession <= 0 && !isGuest) {
      handleLogout();
    }
  ...
  return (
    <>
      ...
      {showExpiredAlert && <SessionExpiredAlert show={showExpiredAlert} onDismiss={handleDismissAlert} />}
    </>
  );
}
